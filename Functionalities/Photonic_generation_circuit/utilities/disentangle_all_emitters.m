...
for l=1:length(disentangled_em)
    ...
    if any(disentangled_em==rest_emitters)
        
        to_remove=[to_remove,l];
...
rest_emitters(to_remove)=[];

for ll=1:length(rest_emitters) 
   
    emitter=rest_emitters(ll);
    ...
    if ~cond_emitter
        ...
        disentangled_em=[disentangled_em,emitter];
...
for ll = 1: length(emm_in_X)
   ...
   Tab     = Had_Gate(Tab,emm_in_X(ll),n);
   Circuit = store_gate_oper(emm_in_X(ll),'H',Circuit,Store_Gates); 
...
emm_in_Z = [emm_in_Z0,emm_in_X(1:ll)];
...
for ll = 1: length(emm_in_Y)
   ...
   Tab     = Phase_Gate(Tab,emm_in_Y(ll),n);
   Tab     = Had_Gate(Tab,emm_in_Y(ll),n);
   
   Circuit = store_gate_oper(emm_in_Y(ll),'P',Circuit,Store_Gates);
   Circuit = store_gate_oper(emm_in_Y(ll),'H',Circuit,Store_Gates);
...
emm_in_Z=[emm_in_Z0,emm_in_X,emm_in_Y];
